...
class this_data:
    def __init__(self, infpn):
        ...
        ['LSU measured Vbr|Temp=20C',
         'By UTokyo MPPC Gain @ bias_voltage=57V|Temp=25C',
         'By UTokyo Noise Rate @ bias_voltage=57V npe=1.5|Temp=25C',
         'By LSU MPPC Gain @ over_voltage=5V|Temp=25C',
         'By LSU Noise Rate @ bias_voltage=57V npe=1.5|Temp=25C',
         'By LSU Uncalibrated Gain @ bias_voltage=57V|Temp=25C',
         'calibration factor from charge injection (electrons/ADC)',
         'By LSU Calibrated Gain @ bias_voltage=57V|Temp=25C',
         'channel']
        '''
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--data_path', type=str, default='LSU_0_25_57.csv')
    ...
    # plot gain
    df = my_data.df_data[['By LSU Calibrated Gain @ bias_voltage=57V|Temp=25C', 'By UTokyo MPPC Gain @ bias_voltage=57V|Temp=25C', 'channel']]
    df = df.melt('channel', var_name='institution',  value_name='MPPC gain')
    g = sns.relplot(x='channel', y='MPPC gain', hue='institution', data=df)
    g.tight_layout()
    g.fig.savefig(f'{out_dir}/mppc_gain_lsu_pcb_lsu_utokyo_temp25.jpg')

    # plot dark noise rate
    # df = my_data.df_data[['By LSU Noise Rate @ bias_voltage=57V npe=1.5|Temp=25C', 'By UTokyo Noise Rate @ bias_voltage=57V npe=0.5|Temp=20C', 'channel']]
    ...
    # g.tight_layout()
    # g.fig.savefig(f'{out_dir}/dark_count_rate_lsu_pcb_lsu_utokyo_temp25.jpg')
