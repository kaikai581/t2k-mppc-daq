#!/usr/bin/env python
'''
This script compares the breakdown voltage measurements of the LSU MPPC-PCB from LSU and UTotyo.
'''
...
if __name__ == '__main__':
    ...
    my_utokyo_data = utokyo_data('data/LSU_PCB_measured_by_UTokyo_crschk_.xlsx')
    ...
    for temp in [20, 25]:
        ...
        ax_lsu20.errorbar(x=df['channel'], y=df['LSU measured Vbr|Temp={}C'.format(temp)], fmt='o')
        ax_lsu20.grid('both')
        ax_lsu20.set_title('LSU PCB\nBreakdown Voltage\nLSU, temperature {}°C'.format(temp))
        ...
        fig.savefig(os.path.join(out_dir, 'Vbd_lsu_pcb_lsu_meas_{}C.png'.format(temp)), bbox_inches='tight')
        ...
        ax_utokyo20.set_title('LSU PCB\nBreakdown Voltage\nUTokyo measurement, temperature {}°C'.format(temp))
        ...
        the_table.set_fontsize(15)
        fig.tight_layout()
        fig.savefig(os.path.join(out_dir, 'Vbd_lsu_pcb_utokyo_meas_{}C.png').format(temp))
        ...
        ax_lsu20_utokyo_20.errorbar(x=df.channel, y=df['LSU measured Vbr|Temp={}C'.format(temp)], c='purple', label='LSU', fmt='o')
        ...
        ax_lsu20_utokyo_20.set_title('LSU PCB\nBreakdown Voltage\ntemperature {}°C'.format(temp))
        ax_utokyo20.figure.tight_layout()
        ax_lsu20_utokyo_20.figure.savefig(os.path.join(out_dir, 'Vbd_lsu_pcb_lsu_meas_utokyo_meas_{}C.png'.format(temp)))
        ...
        ax_lsu20_diff_utokyo_20.set_title('LSU PCB\nBreakdown Voltage\n'+r'LSU$-$UTokyo, temperature {}°C'.format(temp))
        ax_utokyo20.figure.tight_layout()
        ax_lsu20_diff_utokyo_20.figure.savefig(os.path.join(out_dir, 'Vbd_lsu_pcb_lsu_meas_diff_utokyo_meas_{}C.png'.format(temp)))
        ...
        ax_lsu20_ratio_utokyo_20.set_title('LSU PCB\nBreakdown Voltage Ratio\n'+r'LSU$/$UTokyo, temperature {}°C'.format(temp))
        ...
        the_table.set_fontsize(15)
        fig.tight_layout()
        fig.savefig(os.path.join(out_dir, 'Vbd_lsu_pcb_lsu_meas_ratio_utokyo_meas_{}C.png'.format(temp)))