...
from scipy.stats import norm, poisson, gennorm
...
class MPPCLine:
    def __init__(self, infpn, feb_id, ch, prom=300, pc_lth=0.7, pc_rth=1.4, voltage_offset=0, verbose=False, pcb_half=None, exclude_first_peak=False):
        ...
        self.peaks, _ = find_peaks(histy, prominence=prom, wlen=31)
        if len(self.peaks) > 2:
    ...
    def calibrate_absolute_gain(self):
        ...
        self.df_preamp_gain = pd.read_csv('/home/tristan/mppc_analysis/t2k-mppc-daq/FEBDAQMULTx2/data_analysis/preamp_calib_data/caen_measurement.csv')
    ...
    def fit_adc_spectrum(self, func, save_fpn=None):
        ...
        xtalk_frac = .2
        beta = 0.6
        p_init = np.array([N, gain, zero, noise, avnpe, excess, xtalk_frac, beta])
...
class MPPCLines:
    ...
    def save_peaks(self, outfpn):
        columns = ['measurement_id','board','channel','bias_voltage', 'peaks', 'peak_counts']
        if os.path.exists(outfpn):
            df = pd.read_csv(outfpn)
        else:
            df = pd.DataFrame(columns=columns)
        ...
        new_data = dict()
        df_new = pd.DataFrame(columns=columns)
        for elem in self.mppc_lines:
            new_data['measurement_id'] = self.measurement_id
            new_data['board'] = self.feb_id
            new_data['channel'] = self.ch
            new_data['bias_voltage'] = elem.get_bias_voltage()
            new_data['peaks'] = elem.get_peaks()
            new_data['peak_counts'] = elem.get_peak_counts()
            df_new = df_new.append(new_data, ignore_index=True)
        df_new = df_new.set_index(columns[:3])
        print('Saving peaks:')
        print(df_new.head())
...
def gaussian_sum_fit_func(x, N, gain,zero, noise, avnpe, excess, xtalk, beta):
    '''
    ...
    for i in range(maxpe):
        retval += peaksint[i]*(gennorm.pdf(x, beta, peaks[i], math.sqrt(noise**2+i*excess**2)))
    retval *= N
    return retval
